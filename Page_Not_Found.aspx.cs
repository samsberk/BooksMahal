...
public partial class Page_Not_Found : System.Web.UI.Page
{
	...
	protected void nlbtn_Click(object sender, EventArgs e)
	{
		if (nltxt.Text != "")
		{
			...
			if (dt.Rows.Count > 0)
			{
				Response.Redirect("Page_Not_Found?info=" + em.EncryptMyData("AlreadyRegistered") + "");
			}
			else
			{
				...
				if (mm.SendMail(nltxt.Text.ToLower().ToString(), sub, msg))
				{
					...
					{
						Response.Redirect("Page_Not_Found?info=" + em.EncryptMyData("SuccessfullForNewsletter") + "");
					}
					else
					{
						Response.Redirect("Page_Not_Found?info=" + em.EncryptMyData("QueryNotSubmitted") + "");
					}
				}
				else
				{
		...
		{
			Response.Redirect("Page_Not_Found?info=" + em.EncryptMyData("MustEnterYourEmail") + "");
		}
	}
}