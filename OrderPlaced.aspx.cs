...
public partial class OrderPlaced : System.Web.UI.Page
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (Session["orderplaced"] != null)
		{
			...
			ordernolbl.Text = Session["orderplaced"].ToString();
	...
	protected void nlbtn_Click(object sender, EventArgs e)
	{
		if (nltxt.Text != "")
		{
			...
			if (dt.Rows.Count > 0)
			{
				Response.Redirect("Order_Placed_Successfully?info=" + em.EncryptMyData("AlreadyRegistered") + "");
			}
			else
			{
				...
				if (mm.SendMail(nltxt.Text.ToLower().ToString(), sub, msg))
				{
					...
					{
						Response.Redirect("Order_Placed_Successfully?info=" + em.EncryptMyData("SuccessfullForNewsletter") + "");
					}
					else
					{
						Response.Redirect("Order_Placed_Successfully?info=" + em.EncryptMyData("QueryNotSubmitted") + "");
					}
				}
				else
				{
