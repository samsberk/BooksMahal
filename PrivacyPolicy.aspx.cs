...
public partial class PrivacyPolicy : System.Web.UI.Page
{
	...
	protected void nlbtn_Click(object sender, EventArgs e)
	{
		if (nltxt.Text != "")
		{
			...
			if (dt.Rows.Count > 0)
			{
				Response.Redirect("Privacy_Policy?info=" + em.EncryptMyData("AlreadyRegistered") + "");
			}
			else
			{
				...
				if (mm.SendMail(nltxt.Text.ToLower().ToString(), sub, msg))
				{
					...
					{
						Response.Redirect("Privacy_Policy?info=" + em.EncryptMyData("SuccessfullForNewsletter") + "");
					}
					else
					{
						Response.Redirect("Privacy_Policy?info=" + em.EncryptMyData("QueryNotSubmitted") + "");
					}
				}
				else
				{
		...
		{
			Response.Redirect("Privacy_Policy?info=" + em.EncryptMyData("MustEnterYourEmail") + "");
		}
	}
}